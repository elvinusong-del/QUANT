...
from hft_backtest_engine.indicators import realized_volatility # RV 추가
...
class FeatureStore:
    def __init__(
        ...
        vpin_history: int = 180,
        vpin_gate_threshold: float = 0.7,
        vpin_min_buckets: int = 20,   # ✅ 핵심
        tc_window: int = 120,
        ofi_window_minutes: int = 2,
        ofi_z_window: int = 90,
        rv_window: int = 5,          # RV 추가
    ):
        self.symbol = symbol
        ...
        self.rv_window = int(rv_window)      # RV 추가
    ...
    def update_kline(self, kline_df: pd.DataFrame) -> None:
        ...
        self._kline_rows.append(
            {"open_ts": row["open_ts"], "trades": float(row["trades"]),"close": float(row["close"])}
        )
    ...
    def compute_features(self, ts: pd.Timestamp) -> Dict[str, float]:
        ...
        rv_val = 0.0 # RV 추가
        if len(self._kline_rows) > 1:
            # 시간순 정렬
            df_kline = pd.DataFrame(self._kline_rows).sort_values("open_ts")
            # 로그 수익률 계산
            closes = pd.to_numeric(df_kline["close"], errors="coerce")
            returns = np.log(closes / closes.shift(1)).dropna()
            # indicators.py의 함수 사용
            rv_val = realized_volatility(returns, window=self.rv_window)
