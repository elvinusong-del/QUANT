...
class StrategyConfig:
    ...
    max_holding_seconds: int = 5 * 60

    tp_rv_multiplier: float = 1.0      # RV * 1.0 만큼 목표 수익률 설정
    stop_loss_bp: float = 100.0        # -100bp 도달 시 손절
...
class Strategy:
    ...
    def on_tick(self, tick, state: StrategyState) -> List[Order]:
        ...
        # Long(+1)이면 (현재-진입), Short(-1)이면 (진입-현재) -> side * (현재-진입)
        pnl_pct = pos.side * (mid_price - pos.entry_price) / pos.entry_price
        pnl_bp = pnl_pct * 10000.0

        # 2. 손절 기준(-100bp) 체크
        if pnl_bp <= -self.config.stop_loss_bp:
            # 시장가로 전량 청산 주문 생성
            # (참고: 백테스트 엔진은 포지션 청산 시, 해당 심볼의 미체결 지정가 주문들을 자동 취소함)
            orders.append(Order(self.symbol, -pos.side, pos.size, "market", None))
            return orders  # 손절 발생 시 시간 청산 로직은 건너뜀
    ...
    def on_signal(self, tick, state: StrategyState) -> List[Order]:
        ...
        # 1. FeatureStore에서 RV 값 조회
        feats = self.feature_store.get_features(ts)
        rv = feats.get("rv", 0.0)  # RV가 없으면 0.0

        # 2. 목표 수익률 설정 (RV * multiplier), 최소값 보정(예: 5bp)
        target_return = max(rv * self.config.tp_rv_multiplier, 0.0005)
        ...
        # 기존: base_target_bp 고정값 사용 -> 수정: target_return 변수 사용
        
        # TP1 (여기서는 전체 물량에 대해 하나의 TP를 거는 예시로 작성, 분할 청산 원할 시 size * 0.5 등 조절)
        tp_price = price * (1.0 + side * target_return)
        orders.append(Order(self.symbol, -side, size, "limit", tp_price))

        # (필요 없다면 기존 TP2 코드는 삭제 또는 주석 처리)
        # 2) TP1
